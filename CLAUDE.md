...
2. **Wire message models** (`models/`) — `IpcMessage`, a serde-tagged enum of typed payloads (`SpikeBatch`, `EmbeddingBatch`, `StimulusBatch`, `NeuromodulatorSnapshot`, `GradientBatch`, `TraceBatch`, config/control messages) used for structured hybrid-flow messaging. `HybridFlowBackend` is the optional trait for backends that exchange these structured messages (currently no in-repo implementation — `IpcBackend` is the only trait every backend must satisfy).
