...
- A stale doc comment referencing it exists at `src/error.rs:3` (`/// Errors that may be returned by any `TraderBackend` implementation.`). The source audit list has been expanded to include `src/error.rs`.
